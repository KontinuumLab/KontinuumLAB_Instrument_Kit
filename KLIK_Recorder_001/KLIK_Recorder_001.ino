...
// -----------------------------------------------------------------
// Generic recorder with 8 capacitive keys and a CNY70 breathSensor:
// -----------------------------------------------------------------

int activeRecorderKeys[8];
int recorderKeyThreshold = 30;


int octaveSensor = 0;

int keys[8] = {0, 1, 2, 4, 5, 6, 7, 8};

const int recorderFingerings[14][14] = {
  {100, 1, 2, 100, 0, 0, 0, 0, 0, 0, 0, 0, 61}, // C#  
  {2, 100, 1, 100, 0, 0, 0, 0, 0, 0, 0, 0, 60}, // C
  {1, 100, 2, 3, 100, 0, 0, 0, 0, 0, 0, 0, 59}, // B
  {1, 3, 100, 2, 100, 0, 0, 0, 0, 0, 0, 0, 58}, // Bb
  {1, 2, 100, 3, 4, 100, 0, 0, 0, 0, 0, 0, 57}, // A
  {1, 2, 4, 100, 3, 100, 0, 0, 0, 0, 0, 0, 56}, // Ab
  {1, 2, 3, 100, 4, 5, 100, 0, 0, 0, 0, 0, 55}, // G
  {1, 2, 3, 5, 100, 4, 100, 0, 0, 0, 0, 0, 54}, // F#
...
//    Serial.println(breathOut);
  ...
  if(breath != 0){ 
    ...
    for(i = 0; i < 8; i++){
      ...
      lastTouchPinRawVal[keys[i]] = touchPinRawVal[keys[i]];
      // Read sensor at current position:
      touchPinRawVal[keys[i]] = touchRead(touchPin[keys[i]]);
      
      // Filter reading for smoothness:
      touchPinRawVal[keys[i]] = expFilter(touchPinRawVal[keys[i]], lastTouchPinRawVal[keys[i]], 0.01);
//      Serial.print(touchPinRawVal[keys[i]]);
//      Serial.print(" - ");
      // Map reading to MIDI range:
      touchPinVal[keys[i]] = map(touchPinRawVal[keys[i]], touchPinMin[keys[i]], touchPinMax[keys[i]], 0, 127);
      // Limit reading to MIDI range:
      touchPinVal[keys[i]] = constrain(touchPinVal[keys[i]], 0, 127);
//      Serial.print(touchPinVal[keys[i]]);
//      Serial.print(" - ");
      // Activate "activeKeys" at current position, if reading is higher than threshold:
      if(touchPinVal[keys[i]] > recorderKeyThreshold){
        activeRecorderKeys[i] = 1;
      }
      // Else, DEactivate "activeKeys" at current position:
      else{
        activeRecorderKeys[i] = 0;
      }
//      Serial.print(activeRecorderKeys[i]);
//      Serial.print(" - ");
    }
//    Serial.println();
    ...
    if(activeRecorderKeys[octaveSensor] == 1){
      octave = 0;
    }
    else{
      octave = 12;
    ...
    for(n = 0; n < 14; n++){
      ...
      for(i = 0; i < 14; i++){
        if(recorderFingerings[n][i] != 100){
          ...
          if(keyPhase == 0){  // Check "closed" keys
            if(activeRecorderKeys[recorderFingerings[n][i]] == 1){
              correct = 1;
            }
            else{
              correct = 0;
          ...
          // During second keyPhase, check 
            ...
            if(activeRecorderKeys[recorderFingerings[n][i]] == 0){
              correct = 1;
            }
            else{
              correct = 0;
        ...
        else if(recorderFingerings[n][i] == 100){
          if(keyPhase == 1){
            break;
          }
          else{
      ...
      if(correct == 1){
        currentNote = recorderFingerings[n][12];
        break;
      }
    }
