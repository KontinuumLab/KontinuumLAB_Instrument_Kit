...
// Settings pins:
const int optionPin1 = 1;
const int optionPin2 = 3;
const int optionPin3 = 4;

const int instrPin1 = 2;
const int instrPin2 = 5;
const int instrPin3 = 6;
...
void setup() {
  ...
  pinMode(instrPin3, INPUT_PULLUP);
  
  pinMode(optionPin1, INPUT_PULLUP);
  pinMode(optionPin2, INPUT_PULLUP);
  pinMode(optionPin3, INPUT_PULLUP);

  pinMode(calPin, INPUT_PULLUP);
  pinMode(ledPin, OUTPUT);

  modePin1Val = !digitalRead(modePin1);
  modePin2Val = !digitalRead(modePin2);

  instrPin1Val = !digitalRead(instrPin1);
  instrPin2Val = !digitalRead(instrPin2);
  instrPin3Val = !digitalRead(instrPin3);


  bitWrite(modeSetting, 0, modePin1Val);
  bitWrite(modeSetting, 1, modePin2Val);

  bitWrite(instrumentSetting, 0, instrPin1Val);
  bitWrite(instrumentSetting, 1, instrPin2Val);
  bitWrite(instrumentSetting, 2, instrPin3Val);

  
  // All instruments output on alternative MIDI channel:
  if(!digitalRead(optionPin3) == 1){
    MIDIchannel = 2;
  }
